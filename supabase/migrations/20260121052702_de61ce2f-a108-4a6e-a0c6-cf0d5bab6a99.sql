-- Restrict RLS policies to authenticated users only (avoid anonymous access)
...
FOR INSERT
TO authenticated
WITH CHECK (owner_is_unset() AND (owner_user_id = auth.uid()) AND (singleton_id = true));
...
-- Helper macro: owner full access
-- customers
